...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("⚙️ Scoring Control")
...
<ul>
  ...
  <li>Convert probability into <b>risk tiers</b> (Low → Critical).</li>
...
# -----------------------------
    ...
    st.caption("Upload a CSV → score fraud probability → review risk tiers → download results.")
...
# -----------------------------
    ...
    st.caption("Enter key fields → get probability, tier, decision.")
    ...
    run = st.button("🧠 Score transaction", type="primary")
    ...
    if flag == 1:
        st.error("⚠️ Decision: FLAG (send to manual review)")
    else:
...
# -----------------------------
    ...
    st.subheader("🧠 Explainability (Global SHAP)")
